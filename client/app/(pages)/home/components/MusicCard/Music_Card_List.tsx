...
export default function Music_Card_List() {
    ...
    useEffect(() => {
       ...
       // Adjust the scroll speed as needed
            ...
            if (scroll_div.scrollLeft+scroll_div.offsetWidth-scroll_div.scrollWidth==0) {
  ...
  return (
    <div ref={scroll_div_ref} onMouseEnter={() => setIsMouseOver(true)} onMouseLeave={()=> setIsMouseOver(false)} className='  flex p-4  space-x-10  overflow-x-auto scroll-hide '>
    
    {array.map((e,index) =>  <MusicGenre_Card key={index} name={`${index}`} link={'/pop'} color='#ffe200'/>
)}
</div>  )
}
